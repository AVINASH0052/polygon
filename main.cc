...
/* Calculates the convexx difference of two polygons*/
void difference(map<string, ConvexPoligon>& poligons) {
    string line;
    getline(cin, line);
    istringstream iss(line);
    string name;
    ...
    if (identifiers.size() == 3) {
        ...
        auto it3 = poligons.find(identifiers[2]); ConvexPoligon p3 = it3->second;
        auto it2 = poligons.find(identifiers[1]); ConvexPoligon p2 = it2->second;
        auto it1 = poligons.find(identifiers[0]); ConvexPoligon p1 = it1->second;

        if (it1 != poligons.end() && it2 != poligons.end() && it3 != poligons.end()) {
            vector<Point> result = convexHull(p1.puntsDeTall(p2).puntsDeTall(p3));
            ConvexPoligon AUX(result);
            it1->second = AUX;
            // no answer
            cout << "ok" << endl;
        } else cerr << errorReturn[0] << endl;
...
int main(){
  ...
  while(cin >> command){
		...
		else if (command == "difference") difference(poligons);
		else if(command == "#"){
			string comment;
			getline(cin, comment);
			cout << "#" << endl;
